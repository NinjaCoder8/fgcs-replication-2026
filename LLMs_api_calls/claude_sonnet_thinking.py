...
answer = next(block.text for block in response.content if block.type == "text").strip()
print(answer)

text_output = Path("../text_outputs/anthropic/claude_sonnet_thinking")
text_output_path = text_output / "claude_sonnet_thinking_model_results.txt"
...
results_dir = Path("../results/anthropic/claude_sonnet_thinking")
timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')

if df is not None and not df.empty:
    try:
        excel_output_path = results_dir / f"claude_sonnet_thinking_model_results_{timestamp}.xlsx"
